...
export class TableEditCommander implements Commander<TableEditParams> {
  ...
  private static createCell(tagName: string, parser: Parser, rowspan?: number, colspan?: number) {
    ...
    const cell = new Fragment(parser.createFormatDeltasByAbstractData(new AbstractData({
      tag: tagName,
      attrs
    })));
    cell.append(new Single('br', parser.createFormatDeltasByAbstractData(new AbstractData({
      tag: 'br'
    }))));
    return cell;
  }
