...
class KatexComponentLoader implements ComponentLoader {
  ...
  read(element: HTMLElement): ViewData {
    const component = new KatexComponent(element.getAttribute('source'));
    return {
      component,
      slotsMap: []
    }
...
export class KatexComponent extends LeafAbstractComponent {
  ...
  render(): VElement {
    const vEle = new VElement('tb-katex', {
      attrs: {
        source: this.source
      }
    })
    vEle.onRendered = (nativeNode) => {
      Katex.render(this.source, nativeNode, {
        displayMode: true,
        leqno: false,
        fleqn: false,
        throwOnError: true,
