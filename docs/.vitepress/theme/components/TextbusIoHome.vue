...
/** 仅在浏览器挂载编辑器 DOM，避免 SSR HTML 与客户端首帧不一致导致 hydration mismatch */
const editorDomReady = ref(false)

/** XNote 动态导入完成前编辑区保持透明，避免未挂载时出现深色空框 */
const editorSurfaceReady = ref(false)
...
async function bootstrap(): Promise<void> {
  ...
  editor = instance
    ...
    editorSurfaceReady.value = true
  }
}
</script>

<template>
  <div class="tb-io-home" data-allow-mismatch="children">
    <div
        class="static-editor"
    ...
    >
      ...
      <!-- SSR 与 hydration 首帧只渲染占位，真实编辑器仅在客户端挂载后再插入 DOM -->
      <div
        v-if="editorDomReady"
        class="editor"
        :class="{ 'editor--pending': !editorSurfaceReady }"
        :aria-busy="!editorSurfaceReady"
      >
        <div ref="toolbarHost" class="editor-toolbar"/>
        <div ref="contentHost" class="editor-content"/>
      </div>
      <div v-else class="tb-io-home__editor-placeholder" aria-hidden="true"/>
    </div>
...
/* 与真实 .editor 区域等高，仅占位无样式块，避免 SSR/CSR 结构不一致 */
.tb-io-home__editor-placeholder {
  max-width: 880px;
  margin: 0 auto;
  min-height: 362px;
}
