...
下文中的 **方法名、返回值字段名** 与 **`@textbus/core`** 导出一致。插槽与内容流见 [组件基础](./component-basics)；块级属性写入与选区的关系见 [块级样式](./block-styles)；状态查询与命令如何配合选区见 [状态查询与基础操作](./operations-and-query)。
...
手里已经握着 **`Slot` 引用** 时，若只想知道 **它在整篇文档里的「槽位编号」**、还 **没决定光标在该槽第几位**，可以用它只取 **「根 → 该槽」** 这一段，**结果里不含插槽内偏移**。槽不在当前根子树等情况下会得到 **`null`**。
...
这是 **从数字往回找节点**：入参是 **只描述「走到哪一槽」** 的下标序列，**不要** 带上 **`getPaths()`** 里那种 **最后一位偏移**。**`findSlotByPaths`** 若一路能走通且终点是 **插槽**，就返回该 **`Slot`**；**`findComponentByPaths`** 在 **空数组** 时返回 **根组件**，非空时若终点是 **组件** 则返回该 **`Component`**，若终点落在 **插槽** 上则返回 **`null`**，和前者 **互补**。
...
- **`restore(fromLocal)`**：无参等价于 **`restore(true)`**。**`fromLocal`** 表示 **本次选区变更是否由本地文档变更引起**：内核用它区分 **本地** 与 **远程协作** 导致的选区更新。**绝大多数场景**里，选区会随文档改动（无论是本地还是协作）**自动**与内核、页面拖蓝对齐，**不必**手写 **`restore(fromLocal)`**。只有碰到 **少数不走常规管线** 的路径时，才需要自己调用 **`restore`**，并按接入协作时的约定传入 **`true`** / **`false`**；单机编辑一般用无参 **`restore()`** 即可。
