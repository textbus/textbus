...
export class Editor {
  ...
  private getOutputComponentMetadata() {
    ...
    classes.forEach(c => {
      const annotation = getAnnotations(c).getClassMetadata(Component).metadata as Component;
  ...
  private bootstrap(rootInjector: Injector, contentDocument: Document): Injector {
    ...
    this.componentAnnotations = [RootComponent, ...(this.options.components || []), BrComponent].map(c => {
      return getAnnotations(c).getClassMetadata(Component).metadata as Component
    })
    ...
    [RootComponent, ...(this.options.components || [])].forEach(c => {
      ...
      const annotation = metadata.metadata as Component;
