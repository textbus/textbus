...
import { Editor, Member, Organization, TableComponent } from './src/public-api'
import { Selection, Textbus } from '@textbus/core'
import { Message, MessageBus, SyncConnector, YWebsocketConnector } from '@textbus/collaborate'
...
interface MyMessage extends UserSelectionCursor {
  id: string
}

@Injectable()
class MsgBus extends MessageBus<MyMessage> {
  ...
  get(): MyMessage {
    ...
    return {
      ...
      data: c instanceof TableComponent ? c.getSelectedRect() : null
    }
  }

  consume(message: Message<MyMessage>[]) {
    this.collaborateCursor.draw(message.filter(item => {
      return item.message.id !== this.id
    }).map(item => {
      return item.message
    }))
  }
